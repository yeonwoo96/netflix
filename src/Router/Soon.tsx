...
import { DetailMovies, IgetMovies, upcomingMovies } from "../Api";
...
const Soon = () => {
  ...
  const { data: soon, isLoading: Loading3 } = useQuery<IgetMovies>(
    ["movies", "soon"],
    upcomingMovies
  );
  ...
  return (
    <Wrapper>
      {Loading3 ? (
        ...
        <>
          <Banner bgphoto={makeImagePath(soon?.results[0].backdrop_path || "")}>
            <Title
              initial={{ y: -100, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ duration: 1 }}
            >
              {soon?.results[0].title}
            </Title>
            <Overview>{soon?.results[0].overview}</Overview>
            <BtnWrap>
              <Btn>
                <svg
                  width="24"
          ...
          <Slider data={soon!} category="soon" title="방영 예정인 콘텐츠" />
